...
**Status (2026-09-02):** authored, not yet run. `claude plugin eval` answers
...
Each case is `prompt.md` (frontmatter + the prompt) and `graders/*.md`, one
grader per assertion. Deterministic graders wherever possible; an LLM judge only
where the assertion is about wording. The create-mode case is deliberately
