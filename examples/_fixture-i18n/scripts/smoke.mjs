 ...
 * iteration of what tests/preview.spec.ts becomes for /rider-preview
 * in Phase 2.
 *
...
/**
 * How many posts a locale's search index SHOULD hold — counted from the
 * content, never hardcoded.
 *
 * The assertion this feeds is about the locale BOUNDARY: en's index must not
 * contain hu's posts. The count was always incidental to that, and writing it
 * as a literal `4` meant adding one post failed a test that was still checking
 * exactly what it was written to check.
 *
 * The filter is the shared publish predicate, `!draft && !previewOnly`. NOT the
 * `_` filename prefix: Astro's glob loader does not skip those, which
 * /preview/posts/_preview-project serving 200 demonstrates. In this fixture the
 * two rules select the same files, which is exactly how the wrong one survives
 * unnoticed.
 */
function publishedCount(locale) {
  const dir = new URL(`../src/data/blog/${locale}/`, import.meta.url);
  return readdirSync(dir)
    .filter((f) => /\.mdx?$/.test(f))
    .filter((f) => {
      const t = readFileSync(new URL(f, dir), 'utf8');
      return !/^draft:\s*true/m.test(t) && !/^previewOnly:\s*true/m.test(t);
    }).length;
}
...
async function httpSmoke() {
  ...
  const routes = [
    ...
    // NOT /preview/og/en/welcome. `en` is the defaultLocale with
    // prefixDefaultLocale: false, so Astro's dev routing will not serve a URL
    // carrying that segment — while a static build writes
    // dist/preview/og/en/welcome/index.html regardless, because `en` reached
    // getStaticPaths as an ordinary param. Build-only by construction, not a
    // bug to chase: asserting 200 here was a permanently red check (#26). The
    // hu case below exercises the identical route.
...
async function seoSurface() {
  ...
  const checks = [
    ['title tag', /<title>Welcome to the i18n fixture · rider i18n fixture<\/title>/],
...
async function searchFlow(page, locale) {
  ...
  const expectedDocs = publishedCount(locale);
  record(`${locale} · index isolated to locale`, allDocsCount === expectedDocs,
    `got ${allDocsCount} docs (expected ${expectedDocs} published ${locale} post(s))`);
...
await llmsTxt();
await rssCheck();
// Warm the dev server before anything asserts on console errors.
//
// `astro dev` optimises Vite dependencies on first request, and a page loaded
// mid-optimisation can have one dynamic import 404 — which surfaces as
// `Failed to fetch dynamically imported module: /node_modules/.vite/deps/...`
// and fails `en · no console pageerrors`. It passed on a second run every time,
// which is the signature of a warm-up problem rather than a defect, and a suite
// that is red the first time and green the second teaches people to re-run
// until it is green. Discard the first load of each locale instead.
