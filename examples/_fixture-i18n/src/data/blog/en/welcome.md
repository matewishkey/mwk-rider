...
date: 2026-05-27
tags: [meta, fixture]
excerpt: This site exercises the td-rider contract's multi-locale patterns. Two locales, paired translations, locale-only posts. It runs only on localhost.
draft: false
type: thoughts
locale: en
...
This fixture is the test bed for the td-rider contract's §18 multi-locale patterns. It runs only on localhost — never deploys, never has a real R2 bucket. Its job is to give a reviewer a clickable site they can check on a phone before any multi-locale pattern lands on a real managed site like example.com.
