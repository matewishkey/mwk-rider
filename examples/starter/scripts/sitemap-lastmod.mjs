...
/** { '/blog/hello-world': '2026-08-01T00:00:00.000Z', … } */
export function postLastmods(root) {
  const base = join(root, CONTENT_DIR);
  const out = {};
  for (const file of walk(base)) {
    const slug = relative(base, file).replace(/\.mdx?$/, '');
    if (!slug || slug.startsWith('_')) continue;
    const fm = frontmatter(file);
    if (fm.draft === 'true') continue;
    const date = fm.dateModified ?? fm.date;
    if (!date) continue;
    out[`/blog/${slug}`] = new Date(date).toISOString();
  }
  return out;
}
