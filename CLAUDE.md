...
- **Seven offline domains + three `--url` domains**, one module each under `tools/checks/`:
  ...
  - `perf` — `/_astro/*` immutable in `public/_headers`; `<img>` carry width/height (CLS); render-blocking CSS + webfont budgets; heavy third-party embeds behind a facade (`tools/lib/embed-hosts.mjs`) — `loading="lazy"` is not one.
